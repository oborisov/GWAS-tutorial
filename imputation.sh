...
echo '
  rm ${bfile}_ref_chr${i}.phased.impute2 2>/dev/null
  rm ${bfile}_ref_chr${i}.phased.impute2_info 2>/dev/null
  chromosome_min_coord=`tail -n +2 ${dir1000genomes}/genetic_map_chr${chr}_combined_b37.txt | head -1 | awk '{print $1}'`
  chromosome_max_coord=`tail -n -1 ${dir1000genomes}/genetic_map_chr${chr}_combined_b37.txt | awk '{print $1}'`
