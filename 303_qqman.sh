...
snptest_out_dt=rbindlist(sapply(snptest_out_list_files, function(x) { fread(cmd=paste0("zcat ", x, " | grep -v \\#"))[,c("rsid", "chromosome", "position", "info", "cases_maf", "controls_maf", "frequentist_add_pvalue", "frequentist_add_beta_1", "frequentist_add_se_1", "alleleB", "alleleA", "all_total")] }, simplify=F))
