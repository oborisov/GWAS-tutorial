...
# Computing lambda
Rscript -e 'bfile=commandArgs(T); dt=data.table(lambda_4pc=median(qchisq(1 - fread(paste0(bfile, "_lambda_P"), header=F)[[1]], 1), na.rm=T) / qchisq(0.5, 1)); fwrite(dt, paste0(bfile, "_lambda_P"); dt)' ${bfile} > ${bfile}_lambda_P
#####################################################
